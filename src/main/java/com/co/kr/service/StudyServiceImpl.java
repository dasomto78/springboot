...
import com.co.kr.mapper.StudyMapper;
...
public class StudyServiceImpl implements StudyService{
	...
	public int fileProcess(FileListVO fileListVO, MultipartHttpServletRequest request, HttpServletRequest httpReq) {
		...
		//content domain 생성 
		StudyContentDomain studyContentDomain = StudyContentDomain.builder()
				.mbId(session.getAttribute("id").toString())
				.stTitle(fileListVO.getTitle())
				.stContent(fileListVO.getContent())
				.build();
		
				if(fileListVO.getIsEdit() != null) {
					studyContentDomain.setStSeq(Integer.parseInt(fileListVO.getSeq()));
					System.out.println("수정업데이트");
					// db 업데이트
					studyMapper.stContentUpdate(studyContentDomain);
				}else {	
					// db 인서트
					studyMapper.stcontentUpload(studyContentDomain);
					System.out.println(" db 인서트");

				}
				
				// file 데이터 db 저장시 쓰일 값 추출
				int stSeq = studyContentDomain.getStSeq();
				String mbId = studyContentDomain.getMbId();
				...
				for (MultipartFile multipartFile : multipartFiles) {
					...
					if(!multipartFile.isEmpty()) {  // 파일 있을때 
						...
						try {
							...
							//파일 domain 생성 
							StudyFileDomain studyFileDomain = StudyFileDomain.builder()
									.stSeq(stSeq)
									.mbId(mbId)
									.stupOriginalFileName(origFilename)
									.stupNewFileName("resources/upload/"+newFileName) // WebConfig에 동적 이미지 폴더 생성 했기때문
									.stupFilePath(targetPath.toString())
									.stupFileSize((int)multipartFile.getSize())
									.build();
							
								// db 인서트
								studyMapper.stfileUpload(studyFileDomain);
